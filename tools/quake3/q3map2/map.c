...
void LoadEntityIndexMap( entity_t *e ){
	...
	if ( value[ 0 ] == '\0' ) {
		Error( "Entity with index/alpha map \"%s\" has missing \"_layers\" or \"layers\" key\n", indexMapFilename );
		Sys_Printf( "Entity will not be textured properly. Check your keys/values.\n" );
		return;
	}
	numLayers = atoi( value );
	if ( numLayers < 1 ) {
		Error( "Entity with index/alpha map \"%s\" has < 1 layer (%d)\n", indexMapFilename, numLayers );
		Sys_Printf( "Entity will not be textured properly. Check your keys/values.\n" );
		return;
	}
	...
	if ( value[ 0 ] == '\0' ) {
		Error( "Entity with index/alpha map \"%s\" has missing \"_shader\" or \"shader\" key\n", indexMapFilename );
		Sys_Printf( "Entity will not be textured properly. Check your keys/values.\n" );
		return;
	}
	shader = value;
	...
	if ( w < 2 || h < 2 ) {
		Error( "Entity with index/alpha map \"%s\" is smaller than 2x2 pixels\n", indexMapFilename );
		Sys_Printf( "Entity will not be textured properly. Check your keys/values.\n" );
		free( pixels );
		return;
	}
...
static qboolean ParseMapEntity( qboolean onlyLights, qboolean noCollapseGroups ){
	...
	/* conformance check */
	if ( strcmp( token, "{" ) ) {
		Error( "ParseEntity: { not found, found %s on line %d - last entity was at: <%4.2f, %4.2f, %4.2f>...\n"
	...
	while ( 1 )
	{
		...
		if ( !strcmp( token, "{" ) ) {
			...
			if ( !strcmp( token, "patchDef2" ) ) {
				...
				//% ParseTerrain();
				Error( "Terrain entity parsing not supported in this build.\n" ); /* ydnar */
			}
...
void LoadMapFile( char *filename, qboolean onlyLights, qboolean noCollapseGroups ){
	...
	else
	{
		...
		if ( (float) c_detail / (float) numMapBrushes < 0.10f && numMapBrushes > 500 ) {
			Error( "Over 90 percent structural map detected. Compile time may be adversely affected.\n" );
		}
