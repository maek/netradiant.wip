...
static int MakeDecalProjector( shaderInfo_t *si, vec4_t projection, float distance, int numVerts, bspDrawVert_t **dv ){
	...
	if ( numProjectors >= MAX_PROJECTORS ) {
		Error( "MAX_PROJECTORS (%d) exceeded, no more decal projectors available.\n", MAX_PROJECTORS );
		return -2;
	}
...
void ProcessDecals( void ){
	...
	for ( i = 0; i < numEntities; i++ )
	{
		...
		/* any patches? */
		if ( e->patches == NULL ) {
			Error( "Decal entity without any patch meshes, ignoring.\n" );
		...
		/* no target? */
		if ( e2 == NULL ) {
			Error( "Decal entity without a valid target, ignoring.\n" );
			continue;
		}
